...
type Forwarder struct {
	...
	// Allows a custom function to process the packet.
	// If `false` is returned, the packet will NOT be forwarded
	handleCallback	func(conn *net.UDPConn, data []byte, addr *net.UDPAddr) bool
...
func Forward(src, dst string, timeout time.Duration, ch chan string, cb func(conn *net.UDPConn, data []byte, addr *net.UDPAddr) bool) (*Forwarder, error) {
	forwarder := new(Forwarder)
	forwarder.connectCallback = func(addr string) {}
	forwarder.disconnectCallback = func(addr string) {}
	forwarder.handleCallback = cb
	forwarder.connectionsMutex = new(sync.RWMutex)
	forwarder.connections = make(map[string]connection)
	forwarder.channel = ch
...
func (f *Forwarder) handle(data []byte, addr *net.UDPAddr) {
	...
	if !found {
		...
		if f.handleCallback(f.listenerConn, data, addr) {
